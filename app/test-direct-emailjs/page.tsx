...
export default function TestDirectEmailJSPage() {
  ...
  // Test direct EmailJS integration
  const testDirectEmailJS = async () => {
    setIsLoading(true);
    setEmailResult('');
    addLog('Testing direct EmailJS integration...');
    ...
    try {
      ...
      addLog('Sending direct EmailJS to user...');
      ...
      // Send email
      const result = await sendDirectEmailJS(testResult);
      
      // Restore console functions
      console.log = originalConsoleLog;
      console.error = originalConsoleError;

      addLog(`Direct EmailJS result: ${result.success ? 'Success' : 'Failed'}`);
      setEmailResult(JSON.stringify(result, null, 2));
    } catch (error) {
      addLog(`Error testing direct EmailJS: ${error instanceof Error ? error.message : String(error)}`);
  ...
  return (
    <PageLayout>
      <div className="container mx-auto px-6 py-12 max-w-4xl">
        <h1 className="text-3xl font-bold mb-8 text-center">Direct EmailJS Testing</h1>
