...
export default function TestDebugPage() {
  ...
  const testGoogleSheets = async () => {
    ...
    addLog('Testing Google Sheets integration...');
    ...
    try {
      ...
      addLog('Sending test data to Google Sheets...');
      ...
      const result = await sendResultToGoogleSheet(testResult);
      
      // Restore console functions
      console.log = originalConsoleLog;
      console.error = originalConsoleError;

      addLog(`Google Sheets result: ${result.success ? 'Success' : 'Failed'}`);
      setGoogleSheetsResult(JSON.stringify(result, null, 2));
    } catch (error) {
      addLog(`Error testing Google Sheets: ${error instanceof Error ? error.message : String(error)}`);
  ...
  const testEmailJS = async () => {
    setIsLoading(true);
    setEmailResult('');
    addLog('Testing EmailJS integration...');
  ...
  return (
    <PageLayout>
      <div className="container mx-auto px-6 py-12 max-w-4xl">
        <h1 className="text-3xl font-bold mb-8 text-center">Integration Testing</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            >
              {isLoading ? 'Testing...' : 'Test Google Sheets Integration'}
            </button>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <button
              ...
              className="w-full bg-black text-white py-2 rounded mb-4 hover:bg-gray-800 transition-colors"
            >
              {isLoading ? 'Testing...' : 'Test EmailJS Integration'}
            </button>
