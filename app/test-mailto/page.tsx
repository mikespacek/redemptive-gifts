...
export default function TestMailtoPage() {
  ...
  // Test mailto email integration
  const testMailtoEmail = () => {
    setIsLoading(true);
    setEmailResult('');
    addLog('Testing mailto email integration...');
    ...
    try {
      ...
      addLog('Opening mailto email for user...');
      ...
      // Open mailto
      const result = openMailtoEmail(testResult);
      
      // Restore console functions
      console.log = originalConsoleLog;
      console.error = originalConsoleError;

      addLog(`Mailto email result: ${result.success ? 'Success' : 'Failed'}`);
      setEmailResult(JSON.stringify(result, null, 2));
      setIsLoading(false);
    } catch (error) {
      addLog(`Error testing mailto email: ${error instanceof Error ? error.message : String(error)}`);
  ...
  return (
    <PageLayout>
      <div className="container mx-auto px-6 py-12 max-w-4xl">
        <h1 className="text-3xl font-bold mb-8 text-center">Mailto Email Testing</h1>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          <h2 className="text-xl font-bold mb-4">Mailto Email Test</h2>
          <button
            onClick={testMailtoEmail}
            disabled={isLoading}
            className="w-full bg-black text-white py-2 rounded mb-4 hover:bg-gray-800 transition-colors"
          >
            {isLoading ? 'Testing...' : 'Test Mailto Email'}
          </button>
