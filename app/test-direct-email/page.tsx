...
export default function TestDirectEmailPage() {
  ...
  // Test direct email integration
  const testDirectEmail = async () => {
    setIsLoading(true);
    setEmailResult('');
    addLog('Testing direct email integration...');
    ...
    try {
      ...
      addLog('Sending direct email to user...');
      ...
      // Send email
      const result = await sendDirectEmail(testResult);
      
      // Restore console functions
      console.log = originalConsoleLog;
      console.error = originalConsoleError;

      addLog(`Direct email result: ${result.success ? 'Success' : 'Failed'}`);
      setEmailResult(JSON.stringify(result, null, 2));
    } catch (error) {
      addLog(`Error testing direct email: ${error instanceof Error ? error.message : String(error)}`);
  ...
  return (
    <PageLayout>
      <div className="container mx-auto px-6 py-12 max-w-4xl">
        <h1 className="text-3xl font-bold mb-8 text-center">Direct Email Testing</h1>
