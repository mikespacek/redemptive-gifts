 ...
 * This version uses a simpler 24-hour timestamp format
 */
...
/**
 * Format date in simple format: YYYY-MM-DD HH:MM:SS
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (needsFix && jsonData) {
        try {
          // Format the timestamp
          const timestamp = formatDate(jsonData.timestamp || new Date());
